...
export default async function ReportsAdminPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  return <>
    <header className="admin-topbar"><div><span className="admin-pill"><LifeBuoy size={15}/> Safety operations</span><h1>Reports & complaints</h1><p>Student reports, Vendor safety state and Admin decisions are connected through one moderation workflow.</p></div></header>
    ...
    <section className="admin-section"><div className="admin-section-head"><div><h2>Moderation queue</h2><p>Open the connected Vendor or Student record before making a decision when more context is needed.</p></div></div><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Case</th><th>Vendor</th><th>Reporter</th><th>Status</th><th>Update</th></tr></thead><tbody>
      {(complaints || []).map((report)=><tr key={report.id}><td><span className="admin-name">{report.title}</span><span className="admin-sub">{report.description}</span><span className="admin-sub">{new Date(report.created_at).toLocaleString()}</span></td><td>{report.vendor_id ? <Link className="admin-link" href={`/admin-v2/vendors/${report.vendor_id}`}>{vendorMap.get(report.vendor_id) || 'Vendor'}</Link> : 'General report'}</td><td><Link className="admin-link" href={`/admin-v2/students/${report.reporter_id}`}>{reporterMap.get(report.reporter_id) || 'Student'}</Link></td><td><span className={`status-badge status-${report.status}`}>{report.status}</span></td><td><form action={updateComplaint} className="admin-actions"><input type="hidden" name="complaint_id" value={report.id}/><select name="status" defaultValue={report.status} className="admin-action"><option value="open">Open</option><option value="reviewing">Reviewing</option><option value="resolved">Resolved</option><option value="closed">Closed</option></select><button className="admin-action primary" type="submit">Save</button></form></td></tr>)}
