...
async function requireVendorWithSubscription() {
  ...
  const { data: subscriptions, error } = await admin
    .from('subscriptions')
    .select('id,vendor_id,status,provider_subscription_code,created_at,current_period_end')
...
export async function openSubscriptionManager() {
  ...
  try {
    ...
    await admin.from('subscription_events').insert({
      ...
      vendor_id: user.id,
      event_type: 'vendor_opened_subscription_manager',
      source: 'system',
      metadata: { via: 'billing_page' },
    })
...
export async function emailSubscriptionManager() {
  ...
  try {
    ...
    await admin.from('subscription_events').insert({
      subscription_id: subscription.id,
      vendor_id: user.id,
      event_type: 'vendor_requested_subscription_email',
      source: 'system',
      metadata: { via: 'billing_page' },
    })
