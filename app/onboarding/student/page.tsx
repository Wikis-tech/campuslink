...
export default async function StudentOnboardingPage({
  ...
  const hasSchools = Boolean(institutions?.length)
      ...
      <section className="onboarding-layout">
        ...
        <section className="onboarding-panel">
          <div className="panel-heading">
            ...
            <p>Verification improves trust and unlocks campus-specific features, but you can continue to your dashboard and return here later.</p>
          </div>

          <div style={{display:'flex',justifyContent:'flex-end',marginBottom:14}}>
            <Link href="/student" className="btn btn-ghost" style={{display:'inline-flex',alignItems:'center',gap:8}}>Continue to dashboard <ArrowRight size={16}/></Link>
          </div>
          ...
          {!hasSchools ? (
            <div className="auth-success" style={{marginBottom:18}}>
              No schools have been published yet. You can continue to your dashboard now, or request your school below. An admin can add and activate schools from the Phase 4 admin panel.
            </div>
          ) : null}
          ...
          <form action={completeStudentOnboarding} className="onboarding-form">
            <div className="field-block">
              ...
              <select id="institution_id" name="institution_id" required defaultValue={profile?.institution_id || ''} disabled={!hasSchools}>
                <option value="" disabled>{hasSchools ? 'Select your institution' : 'No schools available yet'}</option>
            ...
            <div className="form-grid-2">
              <div className="field-block">
                ...
                <input id="course_of_study" name="course_of_study" required maxLength={120} defaultValue={profile?.course_of_study || ''} placeholder="Computer Science" disabled={!hasSchools} />
              </div>
              <div className="field-block">
                <label htmlFor="study_level">Level</label>
                <select id="study_level" name="study_level" required defaultValue={profile?.study_level || ''} disabled={!hasSchools}>
            ...
            <div className="form-grid-2">
              <div className="field-block">
                ...
                <input id="phone" name="phone" type="tel" required maxLength={30} defaultValue={profile?.phone || ''} placeholder="0800 000 0000" disabled={!hasSchools} />
              </div>
              <div className="field-block">
                <label htmlFor="matric_number">Matric number <span>optional</span></label>
                <input id="matric_number" name="matric_number" maxLength={80} placeholder="Your matric/student number" disabled={!hasSchools} />
              </div>
            </div>

            <fieldset className="verification-choice" disabled={!hasSchools}>
            ...
            <div className="field-block">
              ...
              <input id="school_email" name="school_email" type="email" maxLength={254} defaultValue={profile?.school_email || ''} placeholder="name@school.edu.ng" disabled={!hasSchools} />
            </div>

            <div className="upload-box">
              <div><ShieldCheck size={22} /><strong>Verification document</strong></div>
              <p>PDF, JPG, PNG or WEBP. Maximum 5MB. Stored privately.</p>
              <input name="verification_document" type="file" accept=".pdf,image/jpeg,image/png,image/webp" disabled={!hasSchools} />
            </div>

            <button className="btn btn-primary onboarding-submit" type="submit" disabled={!hasSchools}>Save & submit verification</button>
          </form>

          <details className="school-request" open={!hasSchools}>
