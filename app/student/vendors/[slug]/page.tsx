...
export default async function VendorProfilePage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ error?: string; review?: string; reported?: string }> }) {
  ...
  return <main className="cl-student-page">
    ...
    <section className="cl-student-shell">
        ...
        <div style={{padding:'0 clamp(20px,4vw,44px) 36px'}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',gap:20,alignItems:'flex-start',flexWrap:'wrap',marginTop:16}}>
            ...
            <div style={{display:'grid',gap:9,minWidth:220}}>
              <a className="btn btn-primary" href={`/student/vendors/${vendor.slug}/contact`}><MessageCircle size={17}/> WhatsApp vendor</a>
      ...
      <section className="cl-student-section"><div className="cl-student-section-head"><div><h2>Services</h2><p>What this vendor can do for students.</p></div></div><div className="v3-surface">{(services || []).length ? (services || []).map((service) => <div className="v3-action-row" key={service.id}><span><ShieldCheck size={17}/></span><div><strong>{service.name}</strong><small>{service.description || 'Contact vendor for details.'}</small></div><div style={{display:'grid',justifyItems:'end',gap:6}}><strong>{service.price_from ? `From ₦${Number(service.price_from).toLocaleString()}` : 'Ask vendor'}</strong><a href={`/student/vendors/${vendor.slug}/contact?service=${service.id}&item=${encodeURIComponent(service.name)}`} style={{fontSize:12,fontWeight:850,color:'var(--v3-blue)',textDecoration:'none'}}>Ask about service</a></div></div>) : <p>No detailed services added yet.</p>}</div></section>
