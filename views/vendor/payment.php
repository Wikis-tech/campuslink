...
(function () {
    ...
    async function initiatePayment() {
        ...
        try {
            ...
            // Use authorization_url from server which contains all channel configuration
            openPaystackPopup({ authUrl, reference });
    ...
    function openPaystackPopup({ authUrl, reference }) {
        /* 
         * Use authorization_url directly from server-side initialization.
         * This preserves all server-side configuration including channels (card, bank, ussd, etc).
         * The authorization_url is generated by Paystack API with embedded channel info.
         */
        if (!authUrl) {
            toast('Payment URL not available. Please try again.', 'error');
            return;
        }

        // Paystack inline.js properly handles the authorization_url via a popup
        // Open in a new window or redirect (depending on preference)
        const width = 600;
        const height = 700;
        ...
        // Poll for popup close and refresh CSRF
