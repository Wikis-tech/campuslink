...
.ve-header{background:linear-gradient(135deg,#0b3d91,#1a56db);padding:2rem;color:#fff;}
.ve-icon-wrap{display:flex;justify-content:center;margin-bottom:0.75rem;
animation:iconBounce 1s cubic-bezier(0.34,1.56,0.64,1) both;}
.ve-icon-wrap svg{width:52px;height:52px;stroke:#fff;stroke-width:1.4;fill:none;}
...
.ve-email-box svg{width:15px;height:15px;stroke:#166534;fill:none;stroke-width:2;flex-shrink:0;}
...
.ve-warning svg{width:15px;height:15px;stroke:#92400e;fill:none;stroke-width:2;flex-shrink:0;margin-top:1px;}
.ve-btn{display:flex;align-items:center;justify-content:center;gap:0.45rem;
...
.ve-btn svg{width:15px;height:15px;stroke:#fff;fill:none;stroke-width:2;}
.ve-btn-outline{background:#fff;color:#1a56db;border:1.5px solid #1a56db;margin-top:0.6rem;}
.ve-btn-outline svg{stroke:#1a56db;}
...
.ve-flash{padding:0.65rem 0.85rem;border-radius:9px;font-size:0.82rem;
font-weight:600;margin-bottom:1rem;display:flex;align-items:center;gap:0.5rem;text-align:left;}
.ve-flash svg{width:14px;height:14px;stroke:currentColor;fill:none;stroke-width:2;flex-shrink:0;}
.ve-flash-error{background:#fef2f2;border:1px solid #fecaca;color:#dc2626;}
.ve-flash-success{background:#f0fdf4;border:1px solid #bbf7d0;color:#166534;}
...
<div class="ve-box <?= $status === 'invalid' ? 've-invalid' : '' ?>">
    ...
    <div class="ve-body">
        ...
        <a href="<?= SITE_URL ?>/register" class="ve-btn">
            <i data-lucide="graduation-cap"></i> Register Again
        </a>
        <a href="<?= SITE_URL ?>/login" class="ve-btn ve-btn-outline">
            <i data-lucide="log-in"></i> Already verified? Log In
        </a>
    </div>
    ...
    <div class="ve-body">
        ...
        foreach ($flashes as $type => $messages):
            ...
            foreach ($messages as $message):
                ...
                $isError = ($type === 'error');
                $iconName = $isError ? 'alert-triangle' : 'check-circle';
        ?>
        <div class="ve-flash <?= $isError ? 've-flash-error' : 've-flash-success' ?>">
        ...
        <div class="ve-warning">
            ...
            <span>The verification link expires in <strong>24 hours</strong>.
            If you do not verify within that time you will need to register again.</span>
        </div>
    ...
    <div class="ve-footer">
        Wrong email? <a href="<?= SITE_URL ?>/register">Register with correct email &rarr;</a>
    </div>
